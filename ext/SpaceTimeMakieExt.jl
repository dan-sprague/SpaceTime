...
Plotting helpers that need a Makie backend: `compare_hamiltonian_drift`
(constraint violation along one ray) and `visualize_solution` (an animation
of traced rays around the hole). Loaded automatically when any Makie backend
(CairoMakie, GLMakie, WGLMakie) is imported alongside SpaceTime.
"""
module SpaceTimeMakieExt

using Makie
using SpaceTime
using SpaceTime: AbstractCamera, AbstractSpacetime, Schwarzschild, WorldLine, RayData,
                 RGBf, init_photon, hamiltonian
import SpaceTime: compare_hamiltonian_drift, visualize_solution
const DE = SpaceTime.DifferentialEquations
using .DE: ODEProblem, solve, Vern9
